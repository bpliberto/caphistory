...
model=crm(marked_df, time.intervals = c(1, 1, 1, 3, 4, 5, 2, 3, 1, 4))
model
model=cjs.hessian(model)
model

mark_all <- mark(marked_df, model = "CJS", model.parameters=list(p=list(formula=~1)),output=FALSE)
summary(mark_all)

# mark(marked_df, model = "Huggins", model.parameters=list(p=list(formula=~1)),output=FALSE)$results$beta

```
AIC  :  379.7431, Phi.(Intercept) -0.6325888, p.(Intercept) -1.8079899
...
mod$results$reals
```
sex AIC  :  890.8185, $N  estimate 15.59072
species: AIC  :  1556.97, $N      estimate 3.176451e-15
...
mod$results$reals
```
location: AIC  :  1411.369, $N  estimate  1.58114
species: AIC  :  1556.97, $N      estimate 3.176451e-15
...
model
```
CJS: AIC  :  109.0686, Phi.(Intercept) -0.543118, p.(Intercept)   -1.777340
...
modC$results$reals
```
JS: AIC  :  467.5874, N   estimate= 0.000459042
...
CJS: AIC  :  97.45566, Phi.(Intercept)  0.06849942 , p.(Intercept)   -0.89009108
...
AIC  :  185.9476, N   estimate= 0.09832891
...
```
 ...
 AICc { Phi(~1)p(~1) } = 109.14599 , Phi:(Intercept) -0.5431231, p:(Intercept)   -1.7773484
...
```
 ...
 AICc { Phi(~1)p(~1) } = 97.669942  , Phi:(Intercept)  0.0686517, p:(Intercept)   -0.8902974
...
mark(marked_dfCT, model.parameters=list(Phi=sex.fac, p=sex.fac),output=FALSE)$results$beta
...
# Run with RMark (need to download Program MARK in order for this to work). This is looking at HB only
...
# Run with RMark (need to download Program MARK in order for this to work). This is looking at NMBN only
...
# Run with RMark (need to download Program MARK in order for this to work). This is looking at NMPN only
...
mark(marked_dfNMPS, model = "Jolly", model.parameters=list(p=list(formula=~1)),output=FALSE)$results$beta
```
...
# Run with RMark (need to download Program MARK in order for this to work). This is looking at NCCB only
...
```{r}
capCLSA <- read.csv("capturehistoryCLSA.csv", stringsAsFactors = F)
str(capCLSA)
print(capCLSA)
summary(capCLSA)
capCLSA <- capCLSA[which(!is.na(capCLSA$c1)), ]
summary(capCLSA)
...
data=data.frame(capCLSA)
data
transform=melt(capCLSA, id.vars="mark.number")
pivot=cast(transform, mark.number ~ value)
pivot[is.na(pivot)]=0
...
marked_dfCLSA <- capCLSA

marked_dfCLSA$ch <- paste0(capCLSA$c1,
                capCLSA$c2,
                capCLSA$c3,
                capCLSA$c4,
                capCLSA$c5,
...
```{r}
str(marked_dfCLSA)
marked_dfCLSA$sex.fac <- as.factor(marked_dfCLSA$sex)
marked_dfCLSA$species.fac <- as.factor(marked_dfCLSA$species)
modCLSA <- crm(marked_dfCLSA, 
        model="JS", 
        groups = c("sex.fac"), 
        model.parameters=list(Phi = list(formula = ~sex.fac), 
                              p = list(formula = ~1)))
modCLSA$results$beta #found online but not sure how to use it
modCLSA
...
modCLSA <- crm(marked_dfCLSA, 
        ...
        model.parameters=list(Phi = list(formula = ~species.fac), 
                              p = list(formula = ~species.fac)))
modCLSA$results$beta 
modCLSA
modCLSA$results$reals
```


# Run with RMark (need to download Program MARK in order for this to work). This is looking at CLSA only

```{r}
MarkPath='C:/Program Files (x86)/MARK'
mark(marked_dfCLSA, model.parameters=list(p=list(formula=~1)),output=FALSE)$results$beta
...
```{r}
capNCCP <- read.csv("capturehistoryNCCP.csv", stringsAsFactors = F)
str(capNCCP)
print(capNCCP)
summary(capNCCP)
capNCCP <- capNCCP[which(!is.na(capNCCP$c1)), ]
summary(capNCCP)
...
data=data.frame(capNCCP)
data
transform=melt(capNCCP, id.vars="mark.number")
pivot=cast(transform, mark.number ~ value)
pivot[is.na(pivot)]=0
...
marked_dfNCCP <- capNCCP

marked_dfNCCP$ch <- paste0(capNCCP$c1,
                capNCCP$c2,
                capNCCP$c3,
                capNCCP$c4,
                capNCCP$c5,
...
```{r}
str(marked_dfNCCP)
marked_dfNCCP$sex.fac <- as.factor(marked_dfNCCP$sex)
marked_dfNCCP$species.fac <- as.factor(marked_dfNCCP$species)
modNCCP <- crm(marked_dfNCCP, 
        model="JS", 
        groups = c("sex.fac"), 
        model.parameters=list(Phi = list(formula = ~sex.fac), 
                              p = list(formula = ~1)))
modNCCP$results$beta #found online but not sure how to use it
modNCCP
...
modNCCP <- crm(marked_dfNCCP, 
        ...
        model.parameters=list(Phi = list(formula = ~species.fac), 
                              p = list(formula = ~species.fac)))
modNCCP$results$beta 
modNCCP
modNCCP$results$reals
```


# Run with RMark (need to download Program MARK in order for this to work). This is looking at NCCP only

```{r}
MarkPath='C:/Program Files (x86)/MARK'
mark(marked_dfNCCP, model.parameters=list(p=list(formula=~1)),output=FALSE)$results$beta
...
helpful website: http://www.montana.edu/screel/teaching/bioe-440r-521/course-outline/BIOE_440_CJS_dipper_example.html

I thought maybe the reason why some of these aren't working is because there are no recaps, but I have recaps from CT, NMBS, NCCP, and CLSA. CT and NMBS are the only ones really working, which I guess is okay because those were two of the more important wetlands excluding HB. Again, for HB, it could be that it just can't be calculated because there were never any recaps, but shouldn't I still just be getting some astronomical number for population size? I expect capture probability and apparent survival to be 0, which I can get from RMARK for CJS, but that's not what it's giving me, probably because Phi and p are both equal to 1. 

Also, antoher reason it might not be working for each individual location is because there aren't very many marks, so maybe the CJS/JS where I look at locations and species are good enough, although I am worried that lumping all locations together isn't really giving me what I want. The reason I wanted to split them is because the models assume there is no emigration, so it seems silly to have a bunch of locations together.
...
Should I have species.fac and sex.fac for my CJS models? Why aren't my JS models running for each location? Is this useable data?
