...
dataA$Richness <- dataA %>%  mutate(Richness = Richness / adulteffort) 
dataB$richness <- dataB %>%  mutate(Richness = richness / areasampled) 
dataA$Abundance <- dataA %>%  mutate(Abundance = Abundance / adulteffort) 
dataB$abundance <- dataB %>%  mutate(Abundance = abundance / areasampled)

adultabund <- ggplot(dataA, aes(x=openwater, y=adjusted.abundance)) + geom_point() + xlab("Area of Open Water" ~m^2) + ylab("Adult Abundance")


adultrich <- ggplot(dataA, aes(x=openwater, y=Richness)) + geom_point() + xlab("Area of Open Water" ~m^2) + ylab("Adult Richness")
...
```{r}
library(ggplot2)
nymphabund <- ggplot(dataB, aes(x=openwatersizem2, y=abundance)) + geom_point() + xlab("Area of Open Water" ~m^2) + ylab("Nymph Abundance")


nymphrich <- ggplot(dataB, aes(x=openwatersizem2, y=richness)) + geom_point() + xlab("Area of Open Water" ~m^2) + ylab("Nymph Richness") 
...
dataBdata_comb2 <- dataB %>%
  mutate(Richness1 = richness/ areasampled,
         openwater = openwatersizem2) %>%
  bind_rows(dataA) %>%
  filter(!is.na(Stage))
