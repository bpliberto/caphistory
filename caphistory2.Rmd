...
# make list of models
modelsB <- list(modB6, modB7, modB8, modB9, modB10)
# names of models
...
# make list of models
modelsBR <- list( modBR6, modBR7, modBR8,modBR9, modBR10)
# names of models
...
data_comb <- dataB %>%
  mutate(adjusted.abundance = abundance / areasampled,
         openwater = openwatersizem2) %>%
  bind_rows(dataA) %>%
  filter(!is.na(Stage))
...
data_comb2 <- dataB %>%
  mutate(Richness = richness/ areasampled,
         openwater = openwatersizem2) %>%
  bind_rows(dataA) %>%
  mutate(Richness = Richness / adulteffort)

# both combined with same color
ggplot(data = data_comb2, aes(openwater, Richness)) + geom_point()

# both combined with diff colors
ggplot(data = data_comb2, aes(openwater, Richness)) + geom_point(aes(color = Stage)) + xlab("Area of Open Water" ~(m^2)) + ylab("Richness")

# both on own plots
ggplot(data = data_comb2, aes(openwater, Richness)) + geom_point(aes(color = Stage)) + xlab("Area of Open Water" ~(m^2)) + ylab("Richness") + facet_wrap(~Stage)

# both on own plots + smooth trendline and black-white background
ggplot(data = data_comb2, aes(openwater, Richness)) + geom_point(aes(color = Stage)) + xlab("Area of Open Water" ~(m^2)) + ylab("Richness") + facet_wrap(~Stage) + geom_smooth(method = "lm") + theme_bw()
...
chirodensity <- ggplot(dataF, aes(x=pH, y=chirodensity)) + geom_point() + xlab("pH") + ylab("Chironomid Density") #+ geom_text(aes(label=ID),hjust=.5, vjust=1.5)
